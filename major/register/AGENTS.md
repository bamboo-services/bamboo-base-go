...
节点化注册系统，是整个应用的组件初始化与依赖注入核心。通过 `Register(ctx, nodeList)` 统一完成环境加载、日志器创建、雪花算法初始化、用户自定义节点注册及 Gin 引擎构建。支持 Exec 前注册（`Use`）和 Exec 后动态装配（`UseAfterExec`）两种模式。
...
| Exec 后动态装配组件 | `node/node.go` → `RegNode.UseAfterExec()` | Exec 后调用，立即执行并写入 context（Runner 用） |
...
- **UseAfterExec 用于框架层自动装配**：Runner 在 Exec 后根据 option 动态装配数据库/缓存等内置组件，无需用户手动注册。如果 Exec 尚未执行（`list != nil`），自动退化为 `Use`。
...
3. 启动时 panic "UseAfterExec 执行失败" — 检查 Runner 阶段 option 配置是否正确（如 Redis 地址、数据库 DSN）。
