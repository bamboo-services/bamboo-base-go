...
```
RequestContext → PanicRecovery → HttpLogger → InjectContext → ResponseMiddleware
```
...
```go
data := (&pack.Binding[CreateRequest]{Context: c, GetData: &CreateRequest{}}).Data()    // JSON body
query := (&pack.Binding[ListQuery]{Context: c, GetData: &ListQuery{}}).Query()           // query params
uri := (&pack.Binding[PathParams]{Context: c, GetData: &PathParams{}}).URI()             // URI params
header := (&pack.Binding[HeaderData]{Context: c, GetData: &HeaderData{}}).Header()      // headers
...
自定义验证器在 `common/validator/` 中，包括 `strict_url`、`strict_uuid`、`alphanum_underscore`、`regexp`、`snowflake`、`enum_int`、`enum_string`、`enum_float`。使用 `label` tag 提供中文字段名。
...
拦截器分为 `interceptor/unary/`（一元 RPC）和 `interceptor/stream/`（流式 RPC），自动处理上下文注入、追踪、恢复和中间件。拦截器链执行顺序: `InitContext → Recover → Trace → 用户自定义拦截器 → Middleware`。
