...
// DatabaseInit 根据传入的 [xOptDatabase.DatabaseConfig] 构造数据库初始化节点。
//
...
func openByDriver(cfg xOptDatabase.DatabaseConfig, gormCfg *gorm.Config) (*gorm.DB, error) {
	dsn := cfg.DSN()
	switch cfg.Driver() {
	case xOptDatabase.DriverMySQL:
		return gorm.Open(mysql.Open(dsn), gormCfg)
	case xOptDatabase.DriverPostgres:
		return gorm.Open(postgres.Open(dsn), gormCfg)
	case xOptDatabase.DriverSQLite:
		return gorm.Open(sqlite.Open(dsn), gormCfg)
	case xOptDatabase.DriverOracle:
		return gorm.Open(oracle.Open(dsn), gormCfg)
	case xOptDatabase.DriverSQLServer:
...
//
// 零值字段表示使用默认值，不强制覆盖。
func applyPool(db *gorm.DB, c xOptDatabase.CommonOptions) error {
	sqlDB, err := db.DB()
	if err != nil {
		return err
	}
