...
通用层模块（`github.com/bamboo-services/bamboo-base-go/common`），提供错误处理、日志系统、雪花算法 ID 生成、请求验证器和工具函数。是 `major` 层和 `plugins/*` 插件的共同依赖，被依赖链中的第二层（`defined → common → major`）。
...
│   ├── handler.go             # 自定义 slog Handler（控制台 + 文件双写）
│   ├── gorm.go                # GORM 日志适配器
...
│   ├── translator.go          # 验证错误翻译器（中文）
│   ├── messages.go            # 验证错误消息模板
│   ├── response.go            # HandleValidationError — 统一校验失败响应
...
│   ├── utility.go             # Bind / Encryption / Generate / Password / Str / Timer / Parse / Valid / Function — 工具入口
│   ├── context/               # 上下文工具（xCtxUtil）
│   │   ├── common.go          # IsDebugMode / CalcOverheadTime / GetRequestKey / GetErrorMessage
│   │   ├── database.go        # GetDatabase — 从 context 提取 GORM 实例
│   │   ├── snowflake.go       # GetSnowflakeNode — 从 context 提取雪花节点
│   │   ├── nosql.go           # GetRedis — 从 context 提取 Redis 客户端
│   │   ├── email.go           # GetEmailClient — 从 context 提取邮件客户端
│   │   └── custom.go          # 自定义上下文工具
│   └── package/               # 工具实现（pack）
│       ├── binding.go         # Binding[T] — Data/Query/URI/Header 链式绑定
...
| 绑定请求参数 | `utility/utility.go` → `Bind(ctx, &req)` | 支持 `.Data()` / `.Query()` / `.URI()` / `.Header()` |
| 加密密码 | `utility/package/password.go` → `Encrypt()` | base64 + bcrypt |
| 从 context 获取 DB | `utility/context/database.go` → `GetDatabase()` | 返回 `*gorm.DB` |
| 从 context 获取 Redis | `utility/context/nosql.go` → `GetRedis()` | 返回 `*redis.Client` |
...
- **日志必须从 context 取 trace ID**：`Info/Warn/Error` 系列函数的第一个参数始终是 `ctx context.Context`，Handler 从 context 提取追踪标识。
...
- **Binding 绑定失败返回 nil**：`Bind(ctx, &req).Data()` 失败时返回 `nil`，调用方必须 nil 检查后 return。
...
- **禁止忽略 Binding 返回的 nil** — `Bind().Data()` / `Bind().Query()` 失败返回 nil，不做检查会导致 panic。
- **禁止手动设置 `BaseEntity.ID`** — 雪花 ID 由 `BeforeCreate` 钩子自动生成。

## 调试路径

1. 请求返回错误但日志没有业务错误记录 — 检查 `NewError()` 的 `throw` 参数是否为 `true`。
2. 日志缺少 trace ID — 确认 `InitContext` 中间件或 `RequestContext` helper 已执行。
3. 验证器报错信息是英文 — 确认 `RegisterTranslator()` 在 `RegisterCustomValidators()` 之前调用。
4. 雪花 ID 冲突 — 检查多实例的 `SNOWFLAKE_DATACENTER_ID` + `SNOWFLAKE_NODE_ID` 唯一性。
5. Binding 绑定后数据为 nil — 检查 binding tag 语法（`form:` / `json:` / `binding:`），确认 Content-Type 匹配。
