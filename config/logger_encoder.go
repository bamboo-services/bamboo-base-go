...
// CustomEncoder 是自定义的编码器，用于控制台输出格式
...
// Clone 创建并返回当前 `CustomEncoder` 的深拷贝实例。
//
...
func (e *CustomEncoder) EncodeEntry(entry zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	...
	// 添加日志等级和名称
	customConsoleLevelEncoder(entry.Level, buf)
	customConsoleNameEncoder(entry.LoggerName, buf)
...
// customConsoleLevelEncoder 是一个自定义的日志等级编码器，
// 它会为控制台输出添加颜色和方括号。
func customConsoleLevelEncoder(level zapcore.Level, enc *buffer.Buffer) {
	switch level {
...
func customConsoleNameEncoder(name string, enc *buffer.Buffer) {
	switch name {
	case "CONT", "SERV", "REPO":
