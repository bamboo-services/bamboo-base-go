...
// Test_SnowflakeID_Generate 测试标准雪花 ID 生成
...
// Test_GeneSnowflakeID_Generate 测试基因雪花 ID 生成
func Test_GeneSnowflakeID_Generate(t *testing.T) {
	node, err := xSnowflake.NewGeneNode(1, 1)
	if err != nil {
		t.Fatalf("创建基因雪花节点失败: %v", err)
	}

	id, err := node.Generate(xSnowflake.GeneOrder)
	if err != nil {
		t.Fatalf("生成基因雪花 ID 失败: %v", err)
	}
...
// Test_GeneSnowflakeID_GeneExtraction 测试基因提取
func Test_GeneSnowflakeID_GeneExtraction(t *testing.T) {
	node, _ := xSnowflake.NewGeneNode(0, 0)
...
// Test_GeneSnowflakeID_JSON 测试基因雪花 ID JSON 序列化
func Test_GeneSnowflakeID_JSON(t *testing.T) {
	id := xSnowflake.GeneSnowflakeID(987654321098765432)
	...
	// 反序列化
	var id2 xSnowflake.GeneSnowflakeID
	if err := json.Unmarshal(data, &id2); err != nil {
		t.Fatalf("JSON Unmarshal 失败: %v", err)
	}
	if id2 != id {
...
func Test_NewNode_InvalidParams(t *testing.T) {
	...
	_, err = xSnowflake.NewNode(32, 0)
	if err == nil {
		t.Error("NewNode 应该拒绝超出范围的数据中心 ID")
	}
	...
	_, err = xSnowflake.NewNode(0, 32)
	if err == nil {
		t.Error("NewNode 应该拒绝超出范围的节点 ID")
	}
}

// Test_NewGeneNode_InvalidParams 测试基因节点无效参数
func Test_NewGeneNode_InvalidParams(t *testing.T) {
	// 无效的数据中心 ID
	_, err := xSnowflake.NewGeneNode(-1, 0)
	if err == nil {
		t.Error("NewGeneNode 应该拒绝负数数据中心 ID")
	}

	_, err = xSnowflake.NewGeneNode(8, 0)
	if err == nil {
		t.Error("NewGeneNode 应该拒绝超出范围的数据中心 ID")
	}

	// 无效的节点 ID
	_, err = xSnowflake.NewGeneNode(0, 8)
	if err == nil {
		t.Error("NewGeneNode 应该拒绝超出范围的节点 ID")
	}
}
...
func Test_GlobalFunctions(t *testing.T) {
	...
	// 测试 GenerateGeneID
	geneID, err := xSnowflake.GenerateGeneID(xSnowflake.GeneUser)
	if err != nil {
		t.Fatalf("GenerateGeneID 失败: %v", err)
	}
	if geneID.IsZero() {
		t.Error("GenerateGeneID 不应返回零值")
	}
	if geneID.Gene() != xSnowflake.GeneUser {
		t.Errorf("基因类型 = %d; want %d", geneID.Gene(), xSnowflake.GeneUser)
	}

	// 测试 MustGenerateGeneID
	geneID2 := xSnowflake.MustGenerateGeneID(xSnowflake.GeneOrder)
...
// Test_ParseGeneSnowflakeID 测试解析基因雪花 ID
func Test_ParseGeneSnowflakeID(t *testing.T) {
	original := xSnowflake.MustGenerateGeneID(xSnowflake.GeneProduct)

	// 解析有效字符串
	parsed, err := xSnowflake.ParseGeneSnowflakeID(original.String())
	if err != nil {
		t.Fatalf("ParseGeneSnowflakeID 失败: %v", err)
	}
	if parsed != original {
		t.Errorf("解析结果 = %d; want %d", parsed, original)
	}
...
// Benchmark_SnowflakeID_Generate 基准测试：雪花 ID 生成
...
// Benchmark_GeneSnowflakeID_Generate 基准测试：基因雪花 ID 生成
func Benchmark_GeneSnowflakeID_Generate(b *testing.B) {
	node, _ := xSnowflake.NewGeneNode(0, 0)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = node.Generate(xSnowflake.GeneOrder)
	}
}
