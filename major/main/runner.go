...
// initContext → initOption → initSignal → initSync → initGoroutine → initWeb。
// 各阶段方法分布在同包的不同文件中（option.go / goroutine.go / web.go），
...
// initContext → initOption → initSignal → initSync → initGoroutine → initWeb。
// 各阶段分布在同包的 option.go / goroutine.go / web.go 中，按职责拆分。
//
...
//   - opts 应用级配置选项切片，控制缓存后端、数据库驱动、路由注册等内置组件的装配。
//     传入 nil 或空切片表示不启用任何内置实现，仅以 reg 现有状态启动。
...
func Runner(
	...
	runner := newMainRunner(reg, log, opts)
