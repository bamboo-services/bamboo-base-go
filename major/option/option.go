...
// 用于在 Register 启动阶段声明式地选择框架内置组件的实现（缓存后端、数据库驱动等），
// 而非由业务侧自行编写初始化节点。Option 仅描述「选哪种实现 + 用什么参数」，
// 具体的装配逻辑由 Register 内部根据 [Config] 完成。
//
// 使用示例：
//
//	cfg := option.Apply(
...
// Config 是应用运行期配置的聚合体，由 Register 消费并据此装配内置组件。
//
...
//
// Register 会在 Exec + engineInit 后按此顺序逐个执行，每个 [RouteRegistrar] 接收
